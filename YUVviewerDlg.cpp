...
void CYUVviewerDlg::OnEncrypt() 
{
	...
	if (m_key.GetLength() != 16){
		AfxMessageBox("Please input a 16 bit key.");
	}else if (m_iCount == 0){	
		AfxMessageBox("Open a file first.");
	}else{
	
	UINT picsize = m_nWidth*m_nHeight;
	int lastFile_iCount = m_iCount - 1;

	unsigned char *yEncrypt, *uvEncrypt, *y, *u, *v;
	CFile outf;
	outf.Open("output.e.yuv", CFile::modeCreate | CFile::modeWrite);
	...
	y = (unsigned char*) malloc(picsize);
	yEncrypt = (unsigned char*) malloc(picsize);
	
	u = (unsigned char*) malloc(picsize/4);
	v = (unsigned char*) malloc(picsize/4);
	uvEncrypt = (unsigned char*) malloc(picsize/4);

	std::string s_key((LPCTSTR)m_key);

	if (!m_YY){
			MessageBox("encrypt y");
		}
	if (!m_UU){
			MessageBox("encrypt u");
		}
	if (!m_VV){
			MessageBox("encrypt v");
		}

	int times = 1;
	
	m_pFile[lastFile_iCount]->Seek( 0, CFile::begin );
	while( true )
	{
		if(picsize == m_pFile[lastFile_iCount]->Read(y,picsize))
		{
			if (!m_YY){
				encrypt(y,yEncrypt,picsize,s_key,&times, m_encryptLevel); 
				times=times+1;
				 outf.Write(yEncrypt, picsize);
			}else{
				 outf.Write(y, picsize);
			}
		}else{ break; }
		if(picsize/4 == m_pFile[lastFile_iCount]->Read(u,picsize/4))
		{
			if (!m_UU){
				encrypt(u,uvEncrypt,picsize/4,s_key,&times, m_encryptLevel); 
				times=times+1;
				 outf.Write(uvEncrypt, picsize/4);
			}else{
				outf.Write(u,picsize/4);
			}
		}else{ break; }
		if(picsize/4 == m_pFile[lastFile_iCount]->Read(v,picsize/4))
		{
			if (!m_VV){
				encrypt(v,uvEncrypt,picsize/4,s_key,&times, m_encryptLevel);
				times=times+1;
				 outf.Write(uvEncrypt, picsize/4);
			}else{
				outf.Write(v,picsize/4);
			}
		}else{ break; }
	}

	if(m_pFile[lastFile_iCount]) {
		m_pFile[lastFile_iCount]->Close();
		m_iCount--;
	}
	if(m_pWnd[lastFile_iCount])	m_pWnd[lastFile_iCount]->DestroyWindow();
	if(outf) outf.Close();
...
void CYUVviewerDlg::OnDecrypt() 
{
	...
	CString	key_temp = "abcdefg123456700";
	if (m_key.GetLength() == 16){
		key_temp = m_key;
	}
	if (m_iCount == 0){	
	...
	std::string s_key((LPCTSTR)key_temp);
	...
	while( true )
	{
		...
		if(picsize == m_pFile[lastFile_iCount]->Read(y,picsize))
		{
			if (!m_YY){
				encrypt(y,yEncrypt,picsize,s_key,&times, m_encryptLevel); 
				times=times+1;
				 m_tempFile.Write(yEncrypt, picsize);
			}else{
				 m_tempFile.Write(y, picsize);
			}
		}else{ break; }
		if(picsize/4 == m_pFile[lastFile_iCount]->Read(u,picsize/4))
		{
			if (!m_UU){
				encrypt(u,uvEncrypt,picsize/4,s_key,&times, m_encryptLevel);
				times=times+1;
				 m_tempFile.Write(uvEncrypt, picsize/4);
			}else{
				m_tempFile.Write(u,picsize/4);
			}
		}else{ break; }
		if(picsize/4 == m_pFile[lastFile_iCount]->Read(v,picsize/4))
		{
	...
	if(m_pWnd[lastFile_iCount])	m_pWnd[lastFile_iCount]->DestroyWindow();
	if(m_tempFile) m_tempFile.Close();

	//打开temp文件
	m_pFile[m_iCount] = new CFile();

	if(!m_pFile[m_iCount]->Open(szTempfile, CFile::modeRead )) 
	{
...
void CYUVviewerDlg::pauseAndPlay(){
...
void CYUVviewerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Radio(pDX, IDC_METHOD_Y, m_YY);
	DDX_Radio(pDX, IDC_METHOD_U, m_UU);
	DDX_Radio(pDX, IDC_METHOD_V, m_VV);
	...
	DDX_Text(pDX, IDC_ENCRYPT_LEVEL, m_encryptLevel);
...
BEGIN_MESSAGE_MAP(CYUVviewerDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_METHOD_Y, OnVectorY)
	ON_BN_CLICKED(IDC_METHOD_U, OnVectorU)
	ON_BN_CLICKED(IDC_METHOD_V, OnVectorV)
